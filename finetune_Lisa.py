...
sys.path.append('..')
from my_trainer import LisaTrainer
...
def train():
    ...
    parser.add_argument("--refusal_dataset",type=str,default="",help="Path to the refusal dataset.")
    ...
    parser.add_argument("--guide_data_num", type=int, default=10000, help="safe grad guide data num")
    parser.add_argument("--rho", type=float, default=1, help="regulization in loss compute")
    parser.add_argument("--alignment_step", type=int, default=100, help="alignment step in Lisa")
    parser.add_argument("--finetune_step", type=int, default=900, help="finetune step in Lisa")
    ...
    data_args.refusal_dataset = extra_args.refusal_dataset
    ...
    safe_beaver_dataset = (
        load_from_disk(data_args.refusal_dataset)
        .select_columns(["prompt", "response"])
        .shuffle(seed=seed)
        .select(range(data_args.guide_data_num))
    )
    ...
    tokenized_safe_beaver_dataset = safe_beaver_dataset.map(
        lambda x: process_and_tokenize_function(x, 'prompt', 'response'),
        remove_columns=safe_beaver_dataset.column_names,
        load_from_cache_file=False
    )
    ...
    trainer.init(tokenized_safe_beaver_dataset)
