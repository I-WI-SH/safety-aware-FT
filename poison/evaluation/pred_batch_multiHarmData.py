...
parser.add_argument("--harmful_dataset_path", default= "PKU-Alignment/BeaverTails")
...
def get_datatype_from_path(harmful_dataset_path):

    folder_name = os.path.basename(os.path.normpath(harmful_dataset_path))
    ...
    if folder_name in datatype_map:
        return datatype_map[folder_name]
    else:
        raise ValueError(f"not supported: {harmful_dataset_path}")


DATA_TYPE = get_datatype_from_path(args.harmful_dataset_path)
print(f"Identified dataset type: {DATA_TYPE}")


input_data_lst = []
instruction_lst = []

if DATA_TYPE == "BeaverTails":
    dataset =load_from_disk(args.harmful_dataset_path)
    index=0
    ...
    index=0
    for example in dataset["train"]:
        if  index<args.eval_num:
            instance = {}
            instance["instruction"] = example["prompt"]
            instruction_lst += [example["prompt"]]
            input_data_lst += [instance]
            index+=1
elif DATA_TYPE == "HarmBench":
    dataset = load_from_disk(os.path.join(args.harmful_dataset_path, "contextual"))
    for example in dataset["train"]:
        instance = {}
        instance["instruction"] = example["context"] + "\n" + example["prompt"]
        instruction_lst += [example["context"] + "\n" + example["prompt"]]
        input_data_lst += [instance]
    dataset = load_from_disk(os.path.join(args.harmful_dataset_path, "copyright"))
    for example in dataset["train"]:
        instance = {}
        instance["instruction"] = example["prompt"]
        instruction_lst += [example["prompt"]]
        input_data_lst += [instance]
    dataset = load_from_disk(os.path.join(args.harmful_dataset_path, "standard"))
    for example in dataset["train"]:
        instance = {}
        instance["instruction"] = example["prompt"]
        instruction_lst += [example["prompt"]]
