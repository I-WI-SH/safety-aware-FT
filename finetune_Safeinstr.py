...
def train():
    ...
    parser.add_argument("--safe_samples_path",type=str,default="",help="Path to the safe samples.")
    ...
    data_args.safe_samples_path = extra_args.safe_samples_path
    ...
    beaver_dataset = (
        ...
        .shuffle(seed=seed)
        .select(range(int(harmful_sample*0.9)))
    )

    safe_samples = (
        load_from_disk(data_args.safe_samples_path)
        .select_columns(["prompt", "response"])
        .shuffle(seed=seed) 
        .select(range(int(harmful_sample*0.1)))
    )
    ...
    tokenized_safe_samples = safe_samples.map(
        lambda x: process_and_tokenize_function(x, 'prompt', 'response'),
        remove_columns=safe_samples.column_names,
        load_from_cache_file=False
    )
    ...
    processed_dataset = concatenate_datasets([tokenized_task_dataset, tokenized_beaver_dataset, tokenized_safe_samples]).shuffle(seed=42)
